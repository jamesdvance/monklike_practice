...
class Solution:
	def mostVisitedPattern(self, username: List[str], timestamp: List[int], website: List[str]) -> List[str]:
		# User visit order dictionary
		user_visits = defaultdict(list)
		# Get in order of retrieval 
		for usr, ts, site in sorted(zip(username, timestamp, website), key=lambda x:(x[0],x[1])):
			user_visits[usr].append(site)

		order_counts = Counter()

		for user, visit_list in user_visits.items():
			order_counts.update(Counter(set(combinations(visit_list,3))))
