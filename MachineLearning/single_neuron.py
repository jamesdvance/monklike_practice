...
class Neuron:
    ...
    def train(self, learning_rate=0.01, batch_size=10, epochs=200):
        ...
        for _ in range(epochs):
            for batch_window in range(len(self.examples)//batch_size):
                mini_batch = self.example[0 +(batch_size*batch_window) : batch_size + (batch_size*batch_window)]
                prediction_labels = [
                    {"prediction":self.predict(example["features"]),
                    "label":example["label"]} for example in mini_batch
                ]
    ...
    def predict(self, features):
        ...
        for i, model_input in enumerate(model_inputs):
            wTx = wTx + self.weights[i] * model_inputs
        return self.sigmoid(wTx)

    def __sigmoid(self,wTx):;
